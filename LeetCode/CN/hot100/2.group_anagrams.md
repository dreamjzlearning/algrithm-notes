...
互为异位词的字符串包含的字母相同，那么将字符串的字符排序后结果相同。将排序后的结果作为哈希表的 key，对字符串进行分组。

TC: O(nklogk)

- `n`: 字符串的个数
- `klogk`：k 为字符串最大长度，`klogk`为排序时间
...
互为异位词的字符串包含的字母相同，那么包含的字母数量相同。使用字符计数数组作为 key 进行分组。

TC: *O*(*n*(*k*+∣Σ∣))

- `∣Σ∣`: 字符集大小，题中只包含小写，则为 26
